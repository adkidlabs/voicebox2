...
def build_server(cuda=False, rocm=False):
    ...
    # Bundle the offline preset-voice preview samples generated by
    # scripts/generate_preset_samples.py so the frozen server can serve
    # /preset-samples at runtime. Destination must match where
    # backend/app.py resolves ``__file__/static/preset_samples`` inside the
    # bundle. Source is relative because os.chdir(backend_dir) runs before
    # PyInstaller; absolute paths would bake into the generated .spec.
    samples_src = backend_dir / "static" / "preset_samples"
    if samples_src.is_dir():
        args.append(
            f"--add-data=static/preset_samples{os.pathsep}backend/static/preset_samples"
        )
